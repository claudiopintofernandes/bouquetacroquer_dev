	...
	public function initContent()
	{
		...
		switch ((int)$this->step)
		{
			...
			case OrderController::STEP_ADDRESSES:
				...
				$table_rates = $carrier_table_rate->getTableRates($this->context->language->id, 39);
				foreach($product_list as $product) {
				  if ($product['id_category_default'] == 12 || $product['id_category_default'] == 13) {
					$error_category = 1;
					/* if ($postcode >= 69000 && $postcode <= 69009) {
					  $error_address = 0;
					}
					else {
					  $error_address = 1;
					...
					foreach($table_rates as $table_rate) {
						if (($postcode >= $table_rate['dest_zip_from'] && $postcode <= $table_rate['dest_zip_to']) || $postcode == $table_rate['dest_zip_from']) {
						  $error_address = 0;
						}
					}
				  }
				...
				$this->context->smarty->assign('table_rates', $table_rates);
