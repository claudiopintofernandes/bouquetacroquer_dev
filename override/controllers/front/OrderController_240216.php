...
class OrderControllerCore extends ParentOrderController
{
	...
	public function initContent()
	{
        ...
        $postcodes = Address::getZoneByCustomer($this->context->customer->id);
        $addresses = $this->context->customer->getAddresses((int)Configuration::get('PS_LANG_DEFAULT'));
        $postcode = (int)$addresses[0]['postcode'];
        $product_list = $this->context->cart->getProducts();
        $error_address = 0;
        $error_category = 0;
        foreach($product_list as $product) {
          if ($product['id_category_default'] == 12 || $product['id_category_default'] == 13) {
            $error_category = 1;
            if (($postcode >= 69000 && $postcode <= 69009)||($postcode==69120||$postcode==69130||$postcode==69140||$postcode==69160||$postcode==69160||$postcode==69190||$postcode==69200||$postcode==69250||$postcode==69270||$postcode==69300||$postcode==69330||$postcode==69370||$postcode==69450||$postcode==69500||$postcode==69570||$postcode==69580||$postcode==69650||$postcode==69660||$postcode==69680||$postcode==69730||$postcode==69740||$postcode==69760||$postcode==69800)) {
              $error_address = 0;
            }
            else {
              $error_address = 1;
        ...
        $this->context->smarty->assign('postcodes', $postcodes);
        $this->context->smarty->assign('error_address', $error_address);
        $this->context->smarty->assign('error_category', $error_category);
				$this->_assignAddress();
        ...
        $this->context->smarty->assign('errors_category', $errors_category);
				$this->_assignSummaryInformations();
				$this->setTemplate(_PS_THEME_DIR_.'shopping-cart.tpl');
			break;
		}
