...
class MailCore extends ObjectModel
{
		...
		$template_path = _PS_MAIL_DIR_, $die = false, $id_shop = null, $bcc = null, $reply_to = null)
	{
		...
		try {
			...
			$contactBegin = '<div style="overflow:hidden; margin-bottom:20px; color:#cccccc; line-height:17px"><img width="32" height="32" style="display:block; float:left; margin-right:10px" src="';
			$contactEnd = '</div>';

			$path_to_images = 'http://'.Context::getContext()->shop->domain.Context::getContext()->shop->physical_uri.'themes/'.Context::getContext()->shop->theme_directory.'/mails/images/';

			$template_vars['{adv_img}'] = $template_vars['{social_networks}'] = $template_vars['{email_addr}'] = $template_vars['{email_contacts}'] = ""; 

			if (($emailSettings["email_adv"] == 1) AND ($emailSettings["email_image"] != "")) {
				$template_vars['{adv_img}'] = "<a href='".$emailSettings['email_adv_link']."'><img width='224' border='0' height='224' src='http://".Context::getContext()->shop->domain.Context::getContext()->shop->physical_uri."modules/pk_themesettings/images/upload/".$emailSettings["email_image"]."' alt='' ></a>";
			}
			// social accounts
			if (($emailSettings["email_fb"] == 1) AND ($emailSettings["email_fb_acc"] != "")) {
				$template_vars['{social_networks}'] .= $socialNetworksBegin.$path_to_images.'ic_fb.jpg'.$socialNetworksMid.'href="'.$emailSettings["email_fb_acc"].'">'.Mail::l('Like us on Facebook', (int)Context::getContext()->language->id).$socialNetworksEnd;
			}
			if (($emailSettings["email_tw"] == 1) AND ($emailSettings["email_tw_acc"] != "")) {
				$template_vars['{social_networks}'] .= $socialNetworksBegin.$path_to_images.'ic_tw.jpg'.$socialNetworksMid.'href="'.$emailSettings["email_tw_acc"].'">'.Mail::l('Follow us on Twitter', (int)Context::getContext()->language->id).$socialNetworksEnd;
			}
			if (($emailSettings["email_gp"] == 1) AND ($emailSettings["email_gp_acc"] != "")) {
				$template_vars['{social_networks}'] .= $socialNetworksBegin.$path_to_images.'ic_gp.jpg'.$socialNetworksMid.'href="'.$emailSettings["email_gp_acc"].'">'.Mail::l('Circle us on Google+', (int)Context::getContext()->language->id).$socialNetworksEnd;
			}
			if (($emailSettings["email_yt"] == 1) AND ($emailSettings["email_yt_acc"] != "")) {
				$template_vars['{social_networks}'] .= $socialNetworksBegin.$path_to_images.'ic_yt.jpg'.$socialNetworksMid.'href="'.$emailSettings["email_yt_acc"].'">'.Mail::l('View us on Youtube', (int)Context::getContext()->language->id).$socialNetworksEnd;
			}
