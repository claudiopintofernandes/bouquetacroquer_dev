		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($order_detail_list as $key => $order_detail)
			{
				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					// Ajout date de livraison
					if (Module::isInstalled('eydatepicker'))
					{
					require_once (_PS_ROOT_DIR_."/modules/eydatepicker/eydatepicker.php") ;	
					$models['Deliveryinfo'] = AppModel::loadModel('Deliveryinf');
					$info = $models['Deliveryinfo']->getDeliveryInfo($order->id);
					}
					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
					{
						...
						'{dateofdelivery}' => !empty($info['shipping_date'])?date($this->context->language->date_format_lite, strtotime($info['shipping_date'])):Tools::displayError('NON DEFINI'),
