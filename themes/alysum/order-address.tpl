...
<div class="addresses clearfix">
	...
	{if !$opc}
		<div id="ordermsg" class="form-group">
			...
			<textarea class="form-control" cols="60" rows="6" name="message" placeholder="Message &agrave; joindre au bouquet">{if isset($oldMessage)}{$oldMessage}{/if}</textarea>
		</div>
	{/if}
</div> <!-- end addresses -->
</div>
...
{if !$opc}
		<p class="cart_navigation clearfix">
			...
			<input type="submit" name="processAddress" id="id_submit" class="standard-checkout button btn btn-default button-medium" value="{l s='Proceed to checkout'} &raquo; " {if $error_address == 1}disabled="disabled"{/if} />
		</p>
	</form>
{else}
	</div> <!--  end opc_account -->
{/if}
{addJsDef table_rates=$table_rates}
{addJsDef error_category=$error_category}
