...
class SmartBlogCategories extends Module
{
	...
	public function install()
	{
		...
		if (! parent::install() || ! $this->registerHook('leftColumn') || ! $this->registerHook('actionsbdeletecat') || ! $this->registerHook('actionsbnewcat') || ! $this->registerHook('actionsbupdatecat') || ! $this->registerHook('actionsbtogglecat') || ! $this->registerHook('displaySmartBlogLeft')) return false;
		return true;
	}
	...
	public function hookLeftColumn($params)
	{
		...
		else
		{
			if (! $this->isCached('smartblogcategories.tpl', $this->getCacheId()))
			{
				...
				$categories = $blog_category->getCategory(1, $id_lang);
				$i = 0;
				foreach ($categories as $category)
				{
					$categories[$i]['count'] = $blog_category->getPostByCategory($category['id_smart_blog_category']);
					$i ++;
				}
