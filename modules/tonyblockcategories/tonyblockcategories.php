...
class tonyBlockCategories extends Module
{
	public function __construct()
	{
		$this->name = 'tonyblockcategories';
	...
	public function install()
	{
		if (!parent::install() ||
			...
			!Configuration::updateValue('TONY_BLOCK_CATEG_MAX_DEPTH', 4) ||
			!Configuration::updateValue('TONY_BLOCK_CATEG_DHTML', 1))
			return false;
		return true;
	}
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('TONY_BLOCK_CATEG_MAX_DEPTH') ||
			!Configuration::deleteByName('TONY_BLOCK_CATEG_DHTML'))
			return false;
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitBlockCategories'))
		{
			...
			{
				Configuration::updateValue('TONY_BLOCK_CATEG_MAX_DEPTH', (int)($maxDepth));
				Configuration::updateValue('TONY_BLOCK_CATEG_DHTML', (int)($dhtml));
				Configuration::updateValue('TONY_BLOCK_CATEG_NBR_COLUMN_FOOTER', $nbrColumns);
				Configuration::updateValue('TONY_BLOCK_CATEG_SORT_WAY', Tools::getValue('BLOCK_CATEG_SORT_WAY'));
				Configuration::updateValue('TONY_BLOCK_CATEG_SORT', Tools::getValue('BLOCK_CATEG_SORT'));
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
			<fieldset>
				...
				<div class="margin-form">
					<input type="text" name="maxDepth" value="'.(int)Configuration::get('TONY_BLOCK_CATEG_MAX_DEPTH').'" />
					<p class="clear">'.$this->l('Set the maximum depth of sublevels displayed in this block (0 = infinite)').'</p>
				</div>
				<label>'.$this->l('Dynamic').'</label>

				<div class="margin-form">
					<input type="radio" name="dhtml" id="dhtml_on" value="1" '.(Tools::getValue('dhtml', Configuration::get('TONY_BLOCK_CATEG_DHTML')) ? 'checked="checked" ' : '').'/>
					<label class="t" for="dhtml_on"> <img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" /></label>
					<input type="radio" name="dhtml" id="dhtml_off" value="0" '.(!Tools::getValue('dhtml', Configuration::get('TONY_BLOCK_CATEG_DHTML')) ? 'checked="checked" ' : '').'/>
				...
				<div class="margin-form">
					<input type="radio" name="BLOCK_CATEG_SORT" id="sort_on" value="0" '.(!Tools::getValue('BLOCK_CATEG_SORT', Configuration::get('TONY_BLOCK_CATEG_SORT')) ? 'checked="checked" ' : '').'/>
					...
					<input type="radio" name="BLOCK_CATEG_SORT" id="sort_off" value="1" '.(Tools::getValue('BLOCK_CATEG_SORT', Configuration::get('TONY_BLOCK_CATEG_SORT')) ? 'checked="checked" ' : '').'/>
					...
					<select name="BLOCK_CATEG_SORT_WAY">
						<option value="0" '.(!Tools::getValue('BLOCK_CATEG_SORT_WAY', Configuration::get('TONY_BLOCK_CATEG_SORT_WAY')) ? 'selected="selected" ' : '').'>'.$this->l('Ascending').'</option>
						<option value="1" '.(Tools::getValue('BLOCK_CATEG_SORT_WAY', Configuration::get('TONY_BLOCK_CATEG_SORT_WAY')) ? 'selected="selected" ' : '').'>'.$this->l('Descending').'</option>
					</select>
				</div>
				<label>'.$this->l('How many footer columns would you like?').'</label>
				<div class="margin-form">
					<input type="text" name="nbrColumns" value="'.(int)Configuration::get('TONY_BLOCK_CATEG_NBR_COLUMN_FOOTER').'" />
	...
	{	
		if (!$this->isCached('blockcategories.tpl', $this->getCacheId()))
		{
			...
			$maxdepth = Configuration::get('TONY_BLOCK_CATEG_MAX_DEPTH');
			if (!$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				SELECT DISTINCT c.id_parent, c.id_category, cl.name, cl.description, cl.link_rewrite
				FROM `'._DB_PREFIX_.'category` c
				INNER JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category` AND cl.`id_lang` = '.(int)$this->context->language->id.Shop::addSqlRestrictionOnLang('cl').')
				INNER JOIN `'._DB_PREFIX_.'category_shop` cs ON (cs.`id_category` = c.`id_category` AND cs.`id_shop` = '.(int)$this->context->shop->id.')
				WHERE (c.`active` = 1 OR c.`id_category` = '.(int)Configuration::get('TONY_PS_HOME_CATEGORY').')
				AND c.`id_category` != '.(int)Configuration::get('TONY_PS_ROOT_CATEGORY').'
				'.((int)$maxdepth != 0 ? ' AND `level_depth` <= '.(int)$maxdepth : '').'
				AND c.id_category IN (SELECT id_category FROM `'._DB_PREFIX_.'category_group` WHERE `id_group` IN ('.pSQL($groups).'))
				ORDER BY `level_depth` ASC, '.(Configuration::get('TONY_BLOCK_CATEG_SORT') ? 'cl.`name`' : 'cs.`position`').' '.(Configuration::get('TONY_BLOCK_CATEG_SORT_WAY') ? 'DESC' : 'ASC')))
				return;

			$resultParents = array();
			$resultIds = array();
			$isDhtml = (Configuration::get('TONY_BLOCK_CATEG_DHTML') == 1 ? true : false);
			...
			$blockCategTree = $this->getTree($resultParents, $resultIds, Configuration::get('TONY_BLOCK_CATEG_MAX_DEPTH'));
	...
	public function hookFooter($params)
	{
		...
		if (!$this->isCached('blockcategories_footer.tpl', $this->getCacheId()))
		{
			...
			if (!$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				...
				ORDER BY `level_depth` ASC, '.(Configuration::get('TONY_BLOCK_CATEG_SORT') ? 'cl.`name`' : 'category_shop.`position`').' '.(Configuration::get('TONY_BLOCK_CATEG_SORT_WAY') ? 'DESC' : 'ASC')))
				return;
			$resultParents = array();
			$resultIds = array();
			...
			$nbrColumns = Configuration::get('TONY_BLOCK_CATEG_NBR_COLUMN_FOOTER');
			if (!$nbrColumns)
				$nbrColumns = 3;
