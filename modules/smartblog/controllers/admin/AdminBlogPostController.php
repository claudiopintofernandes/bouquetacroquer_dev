...
class AdminBlogPostController extends AdminController
{
	...
	public function __construct()
	{
		...
		$this->defaultOrderBy = 'a.id_smart_blog_post';
		$this->defaultorderWay = 'DESC';
	...
	public function postProcess()
	{
		...
		elseif (Tools::isSubmit('submitAddsmart_blog_post'))
		{
			...
			{
				$smart_blog_post = new $smart_blog_post();
				...
				foreach ($languages as $language)
				{
					...
					$smart_blog_post->meta_title[$language['id_lang']] = $title;
					$smart_blog_post->meta_keyword[$language['id_lang']] = (string)Tools::getValue('meta_keyword_'.$language['id_lang']);
					$smart_blog_post->meta_description[$language['id_lang']] = Tools::getValue('meta_description_'.$language['id_lang']);
					$smart_blog_post->short_description[$language['id_lang']] = (string)Tools::getValue('short_description_'.$language['id_lang']);
					$smart_blog_post->content[$language['id_lang']] = Tools::getValue('content_'.$language['id_lang']);
					if (Tools::getValue('link_rewrite_'.$language['id_lang']) == '' && Tools::getValue('link_rewrite_'.$language['id_lang']) == null)
						$smart_blog_post->link_rewrite[$language['id_lang']] = str_replace(array(' ', ':', '\\', '/', '#', '!', '*', '.', '?'), '-', Tools::getValue('meta_title_'.$id_lang_default));
					else
						$smart_blog_post->link_rewrite[$language['id_lang']] = str_replace(array(' ', ':', '\\', '/', '#', '!', '*', '.', '?'), '-', Tools::getValue('link_rewrite_'.$language['id_lang']));
				}
				$smart_blog_post->id_parent = Tools::getValue('id_parent');
				$smart_blog_post->position = 0;
				$smart_blog_post->active = Tools::getValue('active');

				$smart_blog_post->id_category = Tools::getValue('id_category');
				$smart_blog_post->comment_status = Tools::getValue('comment_status');
				$smart_blog_post->id_author = $this->context->employee->id;
				$smart_blog_post->created = Date('y-m-d H:i:s');
				$smart_blog_post->modified = Date('y-m-d H:i:s');
				$smart_blog_post->available = 1;
				$smart_blog_post->is_featured = Tools::getValue('is_featured');
				$smart_blog_post->viewed = 1;


				$smart_blog_post->post_type = Tools::getValue('post_type');

				if (! $smart_blog_post->save()) $this->errors[] = Tools::displayError('An error has occurred: Can\'t save the current object');
				else
				{
					Hook::exec('actionsbnewpost', array('SmartBlogPost' => $smart_blog_post));
					$this->updateTags($languages, $smart_blog_post);
					$this->processImage($_FILES, $smart_blog_post->id);
					Tools::redirectAdmin($this->context->link->getAdminLink('AdminBlogPost'));
				}
			}
			elseif ($id_smart_blog_post = Tools::getValue('id_smart_blog_post'))
			{
				...
				$smart_blog_post = new $smart_blog_post($id_smart_blog_post);
				$languages = Language::getLanguages(false);
				foreach ($languages as $language)
				{
					$title = str_replace('"', '', htmlspecialchars_decode(html_entity_decode(Tools::getValue('meta_title_'.$language['id_lang']))));
					$smart_blog_post->meta_title[$language['id_lang']] = $title;
					$smart_blog_post->meta_keyword[$language['id_lang']] = Tools::getValue('meta_keyword_'.$language['id_lang']);
					$smart_blog_post->meta_description[$language['id_lang']] = Tools::getValue('meta_description_'.$language['id_lang']);
					$smart_blog_post->short_description[$language['id_lang']] = Tools::getValue('short_description_'.$language['id_lang']);
					$smart_blog_post->content[$language['id_lang']] = Tools::getValue('content_'.$language['id_lang']);
					$smart_blog_post->link_rewrite[$language['id_lang']] = str_replace(array(' ', ':', '\\', '/', '#', '!', '*', '.', '?'), '-', Tools::getValue('link_rewrite_'.$language['id_lang']));
				}
				$smart_blog_post->is_featured = Tools::getValue('is_featured');
				...
				$smart_blog_post->id_author = $this->context->employee->id;
				$smart_blog_post->modified = Date('y-m-d H:i:s');
				if (! $smart_blog_post->update()) $this->errors[] = Tools::displayError('An error occurred while updating an object.').' <b>'.$this->table.' ('.Db::getInstance()->getMsgError().')</b>';
				else
					Hook::exec('actionsbupdatepost', array('SmartBlogPost' => $smart_blog_post));
				$this->updateTags($languages, $smart_blog_post);
				$this->processImage($_FILES, $smart_blog_post->id_smart_blog_post);
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminBlogPost'));
			}
		}
		...
		{
			$this->defaultOrderBy = Tools::getValue('smart_blog_postOrderby');
			$this->defaultorderWay = Tools::getValue('smart_blog_postOrderway');
		}
	}
	...
	public function processImage($files, $id)
	{
		...
		{
			if (ImageManager::validateUpload($files['image'], 4000000)) return $this->displayError($this->l('Invalid image'));
			else
			{
				...
				if (! move_uploaded_file($files['image']['tmp_name'], $path)) return $this->displayError($this->l('An error occurred while attempting to upload the file.'));
				else
				{
	...
	public function renderForm()
	{
		...
		if (! ($smart_blog_post = $this->loadObject(true))) return;

		$this->fields_form['submit'] = array('title' => $this->l('Save   '), 'class' => 'button');

		$image = ImageManager::thumbnail(_MODULE_SMARTBLOG_DIR_.$smart_blog_post->id_smart_blog_post.'.jpg', $this->table.'_'.(int)$smart_blog_post->id_smart_blog_post.'.'.$this->imageType, 350, $this->imageType, true);

		$this->fields_value = array('image' => $image ? $image : false, 'size' => $image ? filesize(_MODULE_SMARTBLOG_DIR_.$smart_blog_post->id_smart_blog_post.'.jpg') / 1000 : false);
		if (Tools::getvalue('id_smart_blog_post') != '' && Tools::getvalue('id_smart_blog_post') != null)
		{
	...
	{
		$tag_success = true;
		if (! SmartBlogPost::deleteTagsForProduct((int)$post->id)) $this->errors[] = Tools::displayError('An error occurred while attempting to delete previous tags.');
		foreach ($languages as $language) if ($value = Tools::getValue('tags_'.$language['id_lang'])) $tag_success &= SmartBlogPost::addTags($language['id_lang'], (int)$post->id, $value);

		if (! $tag_success) $this->errors[] = Tools::displayError('An error occurred while adding tags.');
		return $tag_success;
	}
}
