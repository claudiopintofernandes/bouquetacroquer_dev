...
class SmartBlog extends Module
{
	...
	public function install()
	{
		...
		Configuration::updateGlobalValue('smartusehtml', '0');
		Configuration::updateGlobalValue('smartshowauthorstyle', '1');
		...
		Configuration::updateGlobalValue('smartshowcolumn', '1');
		...
		if (! parent::install() || ! $this->registerHook('displayHeader') || ! $this->smartHookInsert() || ! $this->registerHook('moduleRoutes') || ! $this->registerHook('displayBackOfficeHeader')) return false;
		$sql = array();
		require_once(dirname(__FILE__).'/sql/install.php');
		foreach ($sql as $sq)
			if (! Db::getInstance()->Execute($sq)) return false;
	...
	{
		$this->smarty->assign(array('smartmodules_dir' => __PS_BASE_URI__));
	...
	private function createSmartBlogTabs()
	{
		...
		$smarttab->save();
		$tab_id = $smarttab->id;
		@copy(dirname(__FILE__).'/img/AdminSmartBlog.gif', _PS_ROOT_DIR_.'/img/t/AdminSmartBlog.gif');
...
<script type="text/javascript">
	...
	{
		$default_lang = (int)Configuration::get('PS_LANG_DEFAULT');
		$this->fields_form[0]['form'] = array('legend' => array('title' => $this->l('Blog Thumblr Configuration'),), 'input' => array(array('type' => 'radio', 'label' => $this->l('Delete Old Thumblr'), 'name' => 'isdeleteoldthumblr', 'required' => false, 'class' => 't', 'is_bool' => true, 'values' => array(array('id' => 'isdeleteoldthumblr', 'value' => 1, 'label' => $this->l('Enabled')), array('id' => 'isdeleteoldthumblr', 'value' => 0, 'label' => $this->l('Disabled'))))

		), 'submit' => array('title' => $this->l('Re Generate Thumblr'), 'class' => 'button'));
	...
	public function processImageUpload($files)
	{
		...
		{
			if (ImageManager::validateUpload($files['avatar'], 4000000)) return $this->displayError($this->l('Invalid image'));
			else
			{
				...
				if (! move_uploaded_file($files['avatar']['tmp_name'], $path)) return $this->displayError($this->l('An error occurred while attempting to upload the file.'));
				else
				{
	...
	public function SampleDataInstall()
	{
		...
		for ($i = 1; $i <= 4; $i ++)
		{
			...
			foreach ($languages as $language)
			{
				if (! Db::getInstance()->Execute('INSERT INTO `'._DB_PREFIX_.'smart_blog_post_lang`(`id_smart_blog_post`,`id_lang`,`meta_title`,`meta_description`,`short_description`,`content`,`link_rewrite`)
	...
	public static function getSmartBlogUrl()
	{
		...
		if ($ssl === null)
		{
			if ($force_ssl === null) $force_ssl = (Configuration::get('PS_SSL_ENABLED') && Configuration::get('PS_SSL_ENABLED_EVERYWHERE'));
			$ssl = $force_ssl;
		}
		if (Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && $id_shop !== null) $shop = new Shop($id_shop);
		else
			$shop = Context::getContext()->shop;
		$base = (($ssl && $ssl_enable) ? 'https://'.$shop->domain_ssl : 'http://'.$shop->domain);
		$lang_url = Language::getIsoById($id_lang).'/';
		if ((! $rewrite_set && in_array($id_shop, array((int)Context::getContext()->shop->id, null))) || ! Language::isMultiLanguageActivated($id_shop) || ! (int)Configuration::get('PS_REWRITING_SETTINGS', null, null, $id_shop)) $lang_url = '';
