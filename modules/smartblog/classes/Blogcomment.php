...
class Blogcomment extends ObjectModel
{
	...
	public static $definition = array('table' => 'smart_blog_comment', 'primary' => 'id_smart_blog_comment', 'multilang' => false, 'fields' => array('id_parent' => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'), 'id_customer' => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'), 'id_post' => array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt'), 'name' => array('type' => self::TYPE_STRING, 'validate' => 'isString'), 'email' => array('type' => self::TYPE_STRING, 'validate' => 'isString'), 'website' => array('type' => self::TYPE_STRING, 'validate' => 'isString'), 'content' => array('type' => self::TYPE_HTML, 'validate' => 'isString'), 'active' => array('type' => self::TYPE_BOOL, 'validate' => 'isBool'), 'created' => array('type' => self::TYPE_DATE, 'validate' => 'isString'),),);
	...
	public function getChildComment($id_parent)
	{
		...
		if (! $child_comments = DB::getInstance()->executeS($sql)) return false;
		$j = 0;
		...
		if (isset($child_comments) && (count($child_comments) > 0))
		{
			foreach ($child_comments as $ch_comment)
			{
				...
				if ($this->comment_child_loop <= $this->comment_child_loop_depth)
				{
					...
					if (count($coments_2) > 0) $child_comments[$j]['child_comments'] = $coments_2;
				}
				$j ++;
	...
	public function getComment($id_post)
	{
		...
		if (! $comments = DB::getInstance()->executeS($sql)) return false;
		$i = 0;
		foreach ($comments as $comment)
		{
			$coments = $this->getChildComment($comment['id_smart_blog_comment']);

			if (count($coments) > 0) $comments[$i]['child_comments'] = $coments;
	...
	public static function getLatestComments($id_lang = null)
	{
		...
		foreach ($sql as $post)
		{
			...
			$result[$i]['slug'] = $smart_blog_post->getPostSlugById($post['id_post']);
			$i ++;
		}
		return $result;
	}
