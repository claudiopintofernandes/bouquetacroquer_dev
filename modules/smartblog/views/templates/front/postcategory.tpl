					...
					<ul class="pagination">
						{if $c!=1}
							...
							<li id="pagination_previous">
								<a href="{smartblog::getSmartBlogLink('smartblog_list_pagination',$options)}" class="small_icon"><i class="icon-left-thin"></i></a>
							</li>
						{/if}
						{for $k=0 to $pagenums}
							{if $title_category != ''}
							...
							{if ($k+1) == $c}
								...
								{/if}
							{/for}
						{if $c!=count($pagenums)+1}
							{assign var="options" value=null}
							{$options.page = $c+1}
							<li id="pagination_next">
								<a href="{smartblog::getSmartBlogLink('smartblog_list_pagination',$options)}" class="small_icon"><i class="icon-right-thin"></i></a>
							</li>
						{/if}
					</ul>
				</div>
