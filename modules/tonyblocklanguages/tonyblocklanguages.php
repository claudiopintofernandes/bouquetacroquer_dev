...
class TonyBlockLanguages extends Module
{

	public function __construct()
	{
		$this->name = 'tonyblocklanguages';
	...
	private function prepareHook($params)
	{
		...
		if (!count($languages)) return false;
		$link = new Link();
		...
		if ((int)Configuration::get('PS_REWRITING_SETTINGS'))
		{
			...
			if (Dispatcher::getInstance()->getController() == 'product' && ($id_product = (int)Tools::getValue('id_product')))
			{
				...
				foreach ($rewrite_infos as $infos) $default_rewrite[$infos['id_lang']] = $link->getProductLink((int)$id_product, $infos['link_rewrite'], $infos['category_rewrite'], $infos['ean13'], (int)$infos['id_lang']);
			}

			if (Dispatcher::getInstance()->getController() == 'category' && ($id_category = (int)Tools::getValue('id_category')))
			{
				$rewrite_infos = Category::getUrlRewriteInformations((int)$id_category);
				foreach ($rewrite_infos as $infos) $default_rewrite[$infos['id_lang']] = $link->getCategoryLink((int)$id_category, $infos['link_rewrite'], $infos['id_lang']);
			}
	...
	{
		if (!$this->prepareHook($params)) return;
		return $this->display(__FILE__, 'views/templates/front/tonyblocklanguages.tpl');
	}
