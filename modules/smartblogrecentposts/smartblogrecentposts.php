...
class SmartBlogRecentPosts extends Module
{
	...
	{
		$ret = parent::install() && $this->registerHook('home') && $this->registerHook('leftColumn') && $this->registerHook('displaySmartBlogLeft') && $this->registerHook('actionsbdeletepost') && $this->registerHook('actionsbnewpost') && $this->registerHook('actionsbupdatepost') && $this->registerHook('actionsbtogglepost');
		if ($ret) {
	...
	public function hookDisplayHome($params)
	{
		...
		{
			if (! $this->isCached('smartblogrecentposts_carousel.tpl', $this->getCacheId()))
			{
				$id_lang = $this->context->language->id;
				$posts = SmartBlogPost::getRecentPosts($id_lang);
				$i = 0;
				foreach ($posts as $post)
				{
					if (file_exists(_PS_MODULE_DIR_.'smartblog/images/'.$post['id_smart_blog_post'].'-single-default.jpg'))
					{
						$image = $post['id_smart_blog_post'];
						$posts[$i]['post_img'] = $image;
					}
			...
			return $this->display(__FILE__, 'views/templates/front/smartblogrecentposts_carousel.tpl', $this->getCacheId());
		}
	}
	...
	public function settingForm()
	{
		...
		$this->fields_form[0]['form'] = array('legend' => array('title' => $this->l('General Setting'),), 'input' => array(array('type' => 'text', 'label' => $this->l('Show Number Of Recent Posts'), 'name' => 'smartshowrecentpost', 'size' => 15, 'required' => true)), 'submit' => array('title' => $this->l('Save'), 'class' => 'button'));
