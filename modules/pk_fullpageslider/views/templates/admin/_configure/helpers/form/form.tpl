			...
			{foreach from=$languages item=language}
				...
				{/if}
					<div class="col-lg-6 active">
						{if isset($input.thumb[$language.id_lang])}
						<img src="{$image_baseurl}{if !empty($input.thumb[$language.id_lang])}{$input.thumb[$language.id_lang]}{else}demo.jpg{/if}" class="img-thumbnail animated" />
						{/if}
				...
				$(document).ready(function(){
					...
					$( "#subimage01_animation" ).change(function() {
						var val = $("option:selected", this).val();
					  $(this).parent().parent().parent().find(".img-thumbnail").removeClass().addClass("img-thumbnail animated "+val);
					});
					$( "#subimage02_animation" ).change(function() {
						var val = $("option:selected", this).val();
					  $(this).parent().parent().parent().find(".img-thumbnail").removeClass().addClass("img-thumbnail animated "+val);
					});
					$( "#subimage03_animation" ).change(function() {
						var val = $("option:selected", this).val();
					  $(this).parent().parent().parent().find(".img-thumbnail").removeClass().addClass("img-thumbnail animated "+val);
					});
				});
