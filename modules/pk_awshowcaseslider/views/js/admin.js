...
/* ON/OFF Switch */
	...
	$('.video_swt').click(function(){
		if($(this).hasClass('active')){
			$(this).removeClass('active').addClass('inactive');
			$(this).children('input').val(0);
			$(this).closest('.image-holder').find('.video-section').addClass('hidden');
			$(this).closest('.image-holder').find('.image-section').removeClass('hidden');
			$(this).closest('.minic-content').find('.img-info').removeClass('hidden');
		}else{
			$(this).removeClass('inactive').addClass('active');
			$(this).children('input').val(1);
			$(this).closest('.image-holder').find('.image-section').addClass('hidden');
			$(this).closest('.image-holder').find('.video-section').removeClass('hidden');
			$(this).closest('.minic-content').find('.img-info').addClass('hidden');
		}
	});

/* Tooltip */
	$('.input-holder .tooltip2').tipsy({
		fade: true,
		gravity: $.fn.tipsy.autoNS,
		offset: 10,
		trigger: 'focus'
	});
	$('label .tooltip2').tipsy({
		fade : true,
		gravity : $.fn.tipsy.autoNS,
		offset : 10,
	});
